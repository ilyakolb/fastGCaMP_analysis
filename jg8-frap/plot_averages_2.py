...
def get_trace_to_plot(trace_array):
    ...
    trace_array_norm = trace_array[:,min_point_idx:]
    trace_array_norm = [np.interp(t, (t.min(), t.max()), (0, 1)) for t in trace_array_norm]
...
(mean, std, t) = get_trace_to_plot(np.array(traces_488['604.2']))
